...
public:
    ...
    std::vector<std::string> remover(
        const std::vector<std::string> bigList,
    ...
    std::vector<std::string> mustHave(
        ...
        std::string must =
        getInput("Type in the letters to include but not sure of location: ");
    ...
    void play() {
        ...
        while (true) {
            ...
            std::string ignore =
            getInput("Type in the letters to ignore (if any): ");

            std::istringstream iss(ignore);

            ignoreWords = std::vector<std::string>(
                (std::istream_iterator<std::string>(iss)),
                std::istream_iterator<std::string>());

            if (!ignoreWords.empty() &&
                !ignoreWords[0].empty()) {
            ...
            std::string nextMove =
            getInput(
            "Do you want to continue (c), reset (r), or exit (e)? ");
